function [ linear ] = emi_linearize(position, expkeys, start_phase, end_phase, boundary1, boundary2)
% Requires:
...
position.data(1,:) = position.data(1,:) ./ conv_cm(1);
position.data(2,:) = position.data(2,:) ./ conv_cm(2);
...
% Set coordinates
coordinates = emi_makecoord(getd(position,'x'), getd(position,'y'), linear_path);
