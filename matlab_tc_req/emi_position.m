...
if exist(['pos-xytime-',unique_id,'.mat'],'file');
    ...
    [pos_x, pos_y, Timestamps] = emi_xy_targets(unique_folder,num_target);
end

[led1on_times, led2on_times, ledoff_times] = emi_lights(unique_folder, expkeys);
...
% Logical array for where lights are on
% lights1 = false(size(pos_x));
% lights1(led1_idx) = true;
% lights2 = false(size(pos_x));
% lights2(led2_idx) = true;
...
% % Converting to cm
% avg_x = avg_x ./ conv_cm(1);
% avg_y = avg_y ./ conv_cm(2);

pos_tsd = emi_pos_tsd(avg_x, avg_y, Timestamps);

if ~CheckTSD(pos_tsd)
    error('tsd check failed.');
end

save([unique_folder(1:15),'-emi-vt.mat'],'pos_tsd');
end
